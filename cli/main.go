// Command kiloproxy is an on-device (Termux) test harness for the
// portable half of the KiloProxy engine: the raw SOCKS5 handshake
// (SocksTester.kt) and the exit-IP verification path
// (Utility.checkWith). Zero dependencies, stdlib only.
//
// It cannot drive the Android-only half (VpnService TUN, tun2socks,
// pdnsd, sendfd): tunnel throughput and packet behavior still need the
// app. Everything here mirrors engine constants so CLI numbers and app
// behavior stay comparable: 5s probe timeouts, 8s checker timeouts,
// kiloip primary + cdn-cgi/trace fallback, IPv4-preferred DNS.
package main
...
func usage() {
	fmt.Fprintf(os.Stderr, `kiloproxy - on-device test harness for the KiloProxy SOCKS5 engine half

Usage: kiloproxy <command> [flags]
...
probe/check/bench/sweep/speed take -host -port -user -pass.
Empty -host means direct (no proxy): the baseline the app compares against.
Run 'kiloproxy <command> -h' for command flags.
`)
}
