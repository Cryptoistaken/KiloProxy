...
- `components/` — Compose components: ConnectionCard, RecentsCard, ProxyCard, ProfileDetailSheet, ProtonControls (ProtonSwitch + ProtonRadio + ProtonDialogRadioRow, mock-exact mono controls), SearchInput, SettingsItem, UpdateDialog, PrefsState (`rememberPref` mirrors a prefs key as state)
  ...
  - `RecentsCard.kt` — Recents list at the bottom of Home: Proton-style rows matching CountriesScreen (flag, name, CODE, right-aligned dial slot swapping for a green dot + Connected label on the active row), "See all" jumps to the Countries tab. Tapping a recent only selects that country (default-profile rewrite via `ProxyProviders.switchCountry` + `Utility.addRecentCountry`); hidden when empty.
