...
- `screens/` — BubbleSettingsScreen, CountriesScreen, DebugLogsScreen, ProxiesScreen, SettingsScreen, SplitTunnelingScreen, StatusScreen, ThemeScreen, VpnAcceleratorScreen
  - `VpnAcceleratorScreen.kt` — Experimental connect-time page (SOCKS5-client scope, not throughput): single `PREF_VPN_ACCELERATOR` toggle, default OFF. Engine honors it (file DNS cache + parallel resolve; verification always fresh via ip-api.com only); OFF = old paths.
